...
interface TooltipInfo { x: number; y: number; node: Node; group: NodeGroup; }
...
export const TreemapView: React.FC<TreemapViewProps> = observer(({
  ...
  const visibleGroups = highlightedGroupId
    ? groups.filter((g) => g.id === highlightedGroupId)
    : groups;
  ...
  const layout = useMemo(() => {
    ...
    const groupItems = visibleGroups.map((g) => ({
      item: g,
      value: Math.max(1, g.nodes.reduce((s, n) => s + nodeSize(n, sizeBy), 0)),
    }));
...
const GroupPanel: React.FC<{ group: NodeGroup; rect: Rect }> = ({ group, rect }) => {
  ...
  return (
    <g>
      ...
      {/* Header text */}
      <text
        className={style.groupHeaderText}
        x={rect.x + GROUP_ACCENT_W + 10}
    ...
    >
      ...
      {w > 40 && h > 40 && (
        <rect
          ...
          rx={CHIP_RADIUS - 2} ry={CHIP_RADIUS - 2}
          fill="none"
...
const CapacityDot: React.FC<{ cx: number; cy: number; cap: "spot" | "on-demand" | "unknown" }> = ({
  cx, cy, cap,
}) => {
...
const PodBar: React.FC<{
  ...
  return (
    <g pointerEvents="none" aria-hidden="true">
      <rect
        className={style.podBarTrack}
        x={x} y={y}
        width={trackW} height={3}
        rx={1.5} ry={1.5}
      />
      {fillW > 0 && (
        <rect
          className={style.podBarFill}
          x={x} y={y}
          width={fillW} height={3}
          rx={1.5} ry={1.5}
        />
      )}
      {showLabel && (
        <text
          className={style.podBarText}
          x={x + width}
          y={y + 3}
          textAnchor="end"
        >
  ...
  return (
    <div className={style.tooltip} style={{ left: info.x + offsetX, top: info.y - 10 }}>
      ...
      <div style={{ marginBottom: 6, display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <span style={{ color: "#8aa0bc", marginLeft: "auto", textTransform: "uppercase", fontSize: 10, letterSpacing: "0.06em" }}>
          {cap}
        </span>
      </div>
...
export const ResponsiveTreemap: React.FC<Omit<TreemapViewProps, "width" | "height">> = (props) => {
  ...
  return (
    <div ref={ref} style={{ width: "100%", height: "100%", minHeight: 520 }}>
      {size.w > 0 && size.h > 0 && (
        <TreemapView {...props} width={size.w} height={size.h} />
      )}
    </div>
  );
};
